  ...
  if (query !== '' &&
    ...
    if (matches.length !== 0)  {
      ...
      if (matches.length === 1
        ...
        if (textAfterCursor(topHit.slice(query.length))) {
        ...
        // if top hit is a CSS property
        if (!topHitHTML.contains('@')) topHitHTML += ':';
...
function textAfterCursor(offset) {
  
  const text = cd.afterCursor();
