...
input.addEventListener('caretmove', onCaretMove);
...
function onType() {
  ...
  const query = cursor.startContainer.textContent.replaceAll(' ','').replaceAll('\n','');
  ...
  if (query !== '' &&
    ...
    if (matches.length !== 0)  {
      ...
      if (matches.length === 1
        ...
        topHitEl.textContent = matches[0].replace(query, '') + ':';
  ...
  if (out.length !== 0) {
    ...
    out.forEach((suggestion, i) => {
      ...
      html += '<div class="icon'+ active +'">' + suggestion.replace(query, '<b>' + query + '</b>') + '</div>';
...
function moveSuggestionsToCaret() {

  const cursor = cd.dropper.cursor();
  const cursorPos = cursor.getBoundingClientRect();

  suggestions.style.left = cursorPos.left - 350 + 'px';
  suggestions.style.top = cursorPos.top + 'px';
  
  topHitEl.style.left = cursorPos.left - 350 + 'px';
  topHitEl.style.top = cursorPos.top + 'px';
