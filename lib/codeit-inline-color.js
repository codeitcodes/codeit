	...
	Prism.hooks.add('wrap', function (env) {
		if (env.type === 'color' || env.classes.indexOf('color') >= 0) {
			...
			if (!env.content.includes('inline-color-wrapper')) {
				...
				var previewElement = '<span class="inline-color-wrapper" aria-hidden="true" contenteditable="false"><span class="inline-color" style="background-color:' + color + ';"></span></span>';
