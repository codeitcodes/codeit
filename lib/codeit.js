...
class CodeitElement extends HTMLElement {
  ...
  constructor() {
    ...
    // drop range at specified coordinates
    cd.dropper.atCoordinates = (x, y) => {
      
      // send new range to x, y coordinates
      let newRange = document.caretRangeFromPoint(x, y);
    ...
    function overrideMouseSelection(event) {
      
      event.preventDefault();
      
      // when selecting with the mouse,
      // the browser handles the selection incorrectly
      // and selection jumps to another position.
      // override with document.caretRangeFromPoint() fixes the problem
      extendSelToCoordinates(event.clientX, event.clientY);
      
    }
    
    function extendSelToCoordinates(x, y) {
      
      // get current selection
      const s = window.getSelection();
      
      // get range for given coordinates
      const endRange = cd.dropper.atCoordinates(x, y);
