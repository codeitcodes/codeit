...
class CodeitElement extends HTMLElement {
  ...
  constructor() {
    ...
    cd.update = () => {
      ...
      if (cd.getSelection().end === cd.textContent.length) {
        /*cd.prev += '\n';
        cd.textContent += '\n';
    ...
    function debounceHighlight() {
      ...
      let debounceTime = 0; //in miliseconds
      if(cd.innerText.length > 5000){
        debounceTime  = 3000;
      }
    ...
    cd.setSelection = (startPos, endPos) => {
      ...
      if( c.startNode.data == '' || c.endNode.data == '' )
      {
        console.log('we have empty nodes',c);
        //If start node and end node of selection are empty and are the same:
    ...
    function getCaretNode(startPos, endPos) {
      ...
      function getCaretTextNode(caretPosInText) {
        ...
        if (lastNode && lastNode.nodeValue) {
          ...
          //If we got to the end of the document:
          if(cd.innerText.length == startPos)
          {
            //cd.textContent += '\n';
            console.log('At end of the document:',startPos, 'endPos:',endPos);

            let i = 0;
            while(i < cd.childNodes.length-1)
            {
              //if( (cd.childNodes[cd.childNodes.length - 1) - i).nodeValue == '' )
              if( (cd.childNodes[(cd.childNodes.length - 1 - i)]).data == '')
              {
                i++;
              }else{
                break;
            ...
            return [cd.childNodes[cd.childNodes.length-2], 0];

          }else{

            console.log('Unable to find node startpos:',startPos, 'endPos:',endPos);
            //If we are in the middle or end of the text start looking for adjacent nodes:
            if(startPos > 1)
            {
