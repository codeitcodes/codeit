...
class CodeitElement extends HTMLElement {
  ...
  constructor() {
    ...
    function getCaretNode(startPos, endPos) {
      ...
      if (endPos && startPos !== endPos) {
      ...
      } else if (startPos !== cd.textContent.length) {
        ...
        const endNode = cd.childNodes[cd.childNodes.length-1];
        
        return {
          startNode: endNode,
          startOffset: endNode.nodeValue.length,
          endNode: endNode,
          endOffset: endNode.nodeValue
        };
