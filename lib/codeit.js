...
let highlightTimeout;//global timeout
...
class CodeitElement extends HTMLElement {
  ...
  constructor() {
    ...
    const modifyEvent = new CustomEvent('modify');
    ...
    function highlightCursorNode() {
      ...
      let highlightHTML = Prism.highlight(elToHighlight.nodeValue,
    ...
    cd.addEventListener('mouseup', checkCaretPosEvent);
    cd.addEventListener('touchend', checkCaretPosEvent);
    cd.addEventListener('keydown', () => { window.setTimeout(checkCaretPosEvent, 0) });
    cd.addEventListener('mousedown', () => { window.setTimeout(checkCaretPosEvent, 0) });
    cd.addEventListener('touchstart', () => { window.setTimeout(checkCaretPosEvent, 0) });
    cd.addEventListener('blur', checkCaretPosEvent);
    ...
    function debounceHighlight() {
      ...
      if (cd.textContent.length > 5000) {
        ...
        // get caret pos in text
<<<<<<< HEAD
        const pos1 = cd.getSelection();

        highlightCursorNode();

        // restore pos in text
        cd.setSelection(pos1.start, pos1.end);


        if(highlightTimeout){
          window.clearTimeout(highlightTimeout);
          console.log('calling again the heavy, but we clear timeout');
        }
        ...
        highlightTimeout = window.setTimeout(() => {
          ...
          console.log('Doing the heavy highlight');
...
>>>>>>> e729722959d0e68ebdf3de0eb03b4b0e80843e58
...
console.log('%ccodeit.js 2.6.5', 'font-style: italic; color: gray');
