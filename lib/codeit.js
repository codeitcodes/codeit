...
class CodeitElement extends HTMLElement {
  ...
  constructor() {
    ...
    function highlightCursorNode() {
      ...
      let elToHighlight = cursor.startContainer.parentElement;
      
      if (elToHighlight.textContent !== '') {

        let highlightLang = Prism.util.getLanguage(elToHighlight);


        let highlightHTML = Prism.highlight(elToHighlight.textContent,
    ...
    cd.on('paste cut', () => { cd.dispatchEvent(modifyEvent) });
