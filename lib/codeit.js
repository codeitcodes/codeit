...
class CodeitElement extends HTMLElement {
  ...
  constructor() {
    ...
    cd.addEventListener('keydown', (event) => {
      ...
      if (cd.options.history) {
        ...
        if (shouldRecord(event) && !recording) {
          ...
          reqNextFrame(() => { cd.dispatchEvent(modifyEvent) });
    ...
    cd.addEventListener('keydown', () => { reqNextFrame(checkCaretPosEvent) });
    cd.addEventListener('mousedown', () => { reqNextFrame(checkCaretPosEvent) });
    cd.addEventListener('mouseup', () => { reqNextFrame(checkCaretPosEvent) });
    cd.addEventListener('touchstart', () => { reqNextFrame(checkCaretPosEvent) }, false);
    cd.addEventListener('touchend', () => { reqNextFrame(checkCaretPosEvent) });
    cd.addEventListener('focus', () => { reqNextFrame(checkCaretPosEvent) });
    cd.addEventListener('blur', () => { reqNextFrame(checkCaretPosEvent) });
...
console.log('%ccodeit.js 2.7.1', 'font-style: italic; color: gray');
