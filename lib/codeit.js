...
class CodeitElement extends HTMLElement {
  ...
  constructor() {
    ...
    cd.highlight = async (lang) => {
      ...
      // remove other language classes from codeit element
      
      const prefix = 'language-';
      
      const classes = cd.className.split(' ')
                        .filter(c => !c.startsWith(prefix));
      
      cd.className = classes.join(' ').trim();


      // add current language class to codeit element
      
      if (lang === null || lang === '') lang = 'plain';
      
      cd.classList.add('language-' + lang);
      
      
      // check if should highlight language
      
      const text = cd.textContent;
      
      const shouldHighlight = cd.shouldHighlight(lang, text);
      
      if (shouldHighlight === false) return;
      
      
      // highlight
      
      const highlightHTML = await cd.highlightText(lang, text);
      
      // if couldn't highlight, return
      if (highlightHTML === false) return;
    ...
    cd.shouldHighlight = (lang, text) => {
      
      const textExists = (text && text !== '' && text !== '\n');
      
      const languageExists = (lang !== 'none' && lang !== 'plain');
      
      return (textExists && languageExists);
      
    }
    
    // highlight text
    // returns highlighted HTML
    cd.highlightText = async (lang, text) => {
      ...
      if (!isLangLoaded(lang)) {
        ...
        // load language
        langLoaded = await loadLanguage(lang);
      ...
      // if couldn't load language, return
      if (!langLoaded) return false;
      
      
      const grammar = Prism.languages[lang];
      
      const highlightHTML = Prism.highlight(text, grammar, lang);
      ...
      return new Promise(resolve => {
        ...
        try {
          ...
          function onError() {
            ...
            // if there was an error in loading the language,
            // return false
            resolve(false);
            
          }
          
          Prism.plugins.autoloader.loadLanguages(lang, onLoad, onError);
    ...
    async function debounceHighlight() {
      ...
      if (cd.textContent && cd.textContent !== '') {
        ...
        if (cd.textContent.length > 5000 &&
          ...
          // set new timeout
          highlightTimeout = window.setTimeout(async () => {

            // get caret pos in text
            const pos = cd.getSelection();

            // highlight codeit
            await cd.highlight(cd.lang);
    ...
    // highlight cursor node
    async function highlightCursorNode() {
