  ...
  constructor() {
    ...
    cd.setSelection = (startPos, endPos) => {
      ...
      if( c.startNode.data == '' || c.endNode.data == '' )
      {
        ...
        {
          s.setBaseAndExtent(c.startNode.previousSibling , c.startNode.previousSibling.textContent.length, c.endNode, c.endOffset);      
        }
      }
      ...
      function getCaretTextNode(caretPosInText) {
        ...
        if (lastNode && lastNode.nodeValue) {
          ...
          if(cd.innerText.length == startPos)
          {
            ...
            return [cd.childNodes[cd.childNodes.length-1], 0];
