  ...
  constructor() {
    ...
    function handleUndoRedo(event) {
      ...
      if (isUndo(event)) {
        ...
        cd.history.at -= 1;
        const record = cd.history.records[cd.history.at];
        ...
        if (cd.history.at < 0) cd.history.at = 0;
      ...
      if (isRedo(event)) {
        ...
        if (cd.history.at >= cd.history.records.length) cd.history.at--;
    ...
    function recordHistory() {
      ...
      const lastRecord = cd.history.records[cd.history.at];
      ...
      cd.history.at++;
      cd.history.records[cd.history.at] = {html, pos};
      cd.history.records.splice(cd.history.at + 1);

      const maxHistory = 300;
      if (cd.history.at > maxHistory) {

        cd.history.at = maxHistory;
