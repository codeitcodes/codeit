...
class CodeitElement extends HTMLElement {
  ...
  constructor() {
    ...
    function debounceHighlight() {
      ...
      if (cd.textContent && cd.textContent !== '' && cd.textContent !== '\n') {
    ...
    function handleDelNewLine(event) {
      ...
      if (event.key === 'Backspace' || event.key === 'Delete') {
        ...
        if (padding.length > 0) {
          ...
          const prevLine = before.slice(-start - padding.length);
          let [prevLinePadding] = getPrevLinePadding(prevLine);
          
          // if the previous line's padding is bigger
          // than the current line's padding, remove only
          // the diff between them
          if (prevLinePadding.length > 0 && prevLinePadding.length < padding.length) {
            
            paddingLength -= prevLinePadding.length;
          ...
          if (pos.start === pos.end && (start + padding.length) === pos.start) {
            ...
            for (let i = 0; i < paddingLength; i++) cd.deleteCurrentSelection();
