...
let git = {
  ...
  'getItems': async (treeLoc) => {
    ...
    // map tree location
    let query = 'https://gitlab.com/api/v4/';
    const [user, repo, contents] = treeLoc;
    ...
    if (repo != '') {
      ...
      query += '/projects/' + user + encodeURL('/') + repoName +
               '/repository/tree?per_page=100&ref=' + branch;

    } else { // else, show all repositories

      query += '/projects?visibility=all&sort=updated&per_page=100&page=1';
